...
from ml.transformers.anomaly_feature_engineer import AnomalyFeatureEngineer
...
class AnomalyDetectionTrainer:
    ...
    def train_with_splits(
        ...
        self.feature_names = self.pipeline.named_steps['anomaly_features'].feature_names_out_
    ...
    def _build_pipeline(self, y_train: pd.Series) -> Pipeline:
        ...
        if self.config.algorithm in ['xgboost', 'lightgbm', 'random_forest']:
            pipeline = Pipeline([
                ('anomaly_features', feature_engineer),
                ('model', model)
            ])
